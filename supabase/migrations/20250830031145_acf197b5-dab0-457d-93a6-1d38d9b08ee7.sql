-- Fix RLS policies for enrollments to allow teachers to enroll students in their classes
DROP POLICY IF EXISTS "Teachers can create enrollments for their classes" ON public.enrollments;
DROP POLICY IF EXISTS "Teachers can enroll students in their classes" ON public.enrollments;
...
-- Fix the unenroll function to properly work with class names
...
-- Ensure admin_update_student_data function exists with complete implementation
