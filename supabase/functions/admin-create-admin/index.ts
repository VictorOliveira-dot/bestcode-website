...
serve(async (req) => {
  ...
  try {
    ...
    const { data: newUser, error: createUserError } = await supabaseAdmin.auth.admin.createUser({
      ...
      password,
      user_metadata: {
        name: email.split('@')[0], // Use parte antes do @ como nome padrão
        role: "admin"
      },
    ...
    console.log("Administrador inserido na public.users");
    ...
    console.error("Erro na função admin-create-admin:", error);
